import { Class, Filter, MapReflector, upperCaseFirst } from '@codeshine/nestjs-query-core'
  ...
  return reflector.memoize(TClass, typeName, () => {
    ...
    if (depth > 0) {
      Object.keys(filterableRelations).forEach((field) => {
        ...
        if (FieldType) {
          const FC = getOrCreateFilterType(FieldType, newPrefix, suffix, depth - 1, true)
    ...
    if (isRelation) {
      const FC = getOrCreateFilterType(TClass, null, 'ExistsNotExists', depth)
